...
SERIALIZABLE(STRUCT(bt_uuid));
	SERIALIZABLE(EXTRA(u8_t *data));
	SERIALIZABLE(EXTRA(size_t size));
	SERIALIZABLE(ARRAY_LENGTH(data, size));
	SERIALIZABLE(ARRAY_MAX_LENGTH(data, 16));
	SERIALIZABLE(TODO("decode/encode rest of bt_uuid_xx using extra variables"));
...
struct bt_gatt_attr {
	...
	SERIALIZABLE(EXTRA(u8_t *data));
	SERIALIZABLE(TODO("known independent read/write callbacks replace on NET side to handle them on NET side only"));
...
struct bt_gatt_service_static {
	...
	SERIALIZABLE(ARRAY_LENGTH(attrs, attrs_count));
...
int bt_gatt_service_register(struct bt_gatt_service *svc);
	SERIALIZABLE(EXTRA(void *ptr));
	SERIALIZABLE(TODO("Save pointer using ptr. The same pointer will be used to unregister"));
	SERIALIZABLE(TODO("Decoder: new service have to be allocated"));
	SERIALIZABLE(TODO("Add attribute to attribute translation table"));
	/*
	bt_gatt_attr *attr;
	serializable_attr sattr;
	bt_gatt_attr_block *block = get_attr_block(attr);
	sattr.block_number = block->number;
	sattr.attr_index = attr - block->attrs;
	*/
...
int bt_gatt_service_unregister(struct bt_gatt_service *svc);
	SERIALIZABLE(POINTER_VALUE(svc));
...
typedef u8_t (*bt_gatt_attr_func_t)(const struct bt_gatt_attr *attr,
				       void *user_data);
	SERIALIZABLE(CONVERT(attr, attr_pointer_converter));
...
ssize_t bt_gatt_attr_read(struct bt_conn *conn, const struct bt_gatt_attr *attr,
	...
	NOT_SERIALIZABLE("Execute on APP side");
...
ssize_t bt_gatt_attr_read_service(struct bt_conn *conn,
	...
	NOT_SERIALIZABLE("Used as callback in macro, shouldn't be used directly");
...
struct _bt_gatt_ccc {
	...
	SERIALIZABLE(IGNORE(cfg));
	SERIALIZABLE(TODO("Callbacks"));
...
u32_t callback_index[256];


__attribute__((naked))
void callback_jump_entry_1(void *ptr)
{
	__asm(
		"jump_table_start:\n"
		...
		push r4
		ldr r4, jump_table_start_addr_value
		sub lr, r4
		shift lr, #2
		ldr r4, callback_index_addr_value
		disable intr
		str lr, [r4]
		add lr, r4
		ldr lr, [r4]

		ldr r0, jump_table_start_addr

		jump_table_start_addr_value:
		.word jump_table_start
		"
		);
